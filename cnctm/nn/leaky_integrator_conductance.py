...
class leaky_integrator_conductance(Module):
    ...
    """
    def __init__(self, connectome, dt, hidden_init=None, hidden_init_trainable=False,  bias_init=None, tau_init=None, nonlinearity=F.relu, constraint = 'weight', is_Training = True, gener = 0):
        ...
        magnitude_scaling_factor_chem = 5e-1 if gener else 1e-2
        magnitude_scaling_factor_elec = 5e-1 if gener else 1e-2
        
        if gener:
            self.Eexc = Parameter(torch.tensor(connectome.Eexc))
            self.Einh = Parameter(torch.tensor(connectome.Einh))
            self.excMask = connectome.Ejinit[None] == connectome.Eexc
            self.inhMask = connectome.Ejinit[None] == connectome.Einh

            excRatio = torch.empty(1,self.n,self.n).uniform_(-0.05, 0.05) + 0.5
            excRatio[self.excMask] = 1
            excRatio[self.inhMask] = 0
            self.excRatio = Parameter(excRatio)
        else:
        ...
        if gener:
            self.bias = Parameter(torch.empty(1,self.n).normal_(connectome.Eleaky, 0.1))
        elif bias_init is None:
            self.bias = Parameter(torch.empty(self.n).uniform_(0.01, 0.02))
        else:
            self.bias = Parameter(bias_init)
        if gener:
            self.tau = Parameter(torch.empty(1,self.n).normal_(0.1, 0.01))
        elif tau_init is None:
    ...
    def Reversal_potentials(self):
        if self.gener:
            return (self.excRatio * self.Eexc + (1 - self.excRatio) * self.Einh)
        else:
            return self.reversal_potentials
