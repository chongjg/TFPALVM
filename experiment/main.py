...
parser.add_argument("--neuron_holdout", default = [], nargs='+', type=str, help="neuron holdout list")
parser.add_argument("--train_worm", default = [1], nargs='+', type=int, help="train worm list")
...
parser.add_argument("--infer", default = 0, type=int, help="inference network type, Temporal-filter -- 1 : enable, 0 : disable")
parser.add_argument("--gener", default = 0, type=int, help="generative network type, Physiology-aligned -- 1 : enable, 0 : disable")
args = parser.parse_args()

exp_name =  f'worm_vae_neuron_held_out_{args.neuron_holdout}_train_worm_{args.train_worm}_{args.model_type}_{args.constraint}_rand_{str(args.random_init_index)}_infer_{args.infer}_gener_{args.gener}'

logging.basicConfig(filename = 'logs/'+exp_name+'logging.log',level=logging.DEBUG,format='%(message)s')

print('neuron_holdout:',str(args.neuron_holdout))
print('train_worm:',str(args.train_worm))
print('model_type:',str(args.model_type))
print('constraint:',str(args.constraint))
print('random_init_index:',str(args.random_init_index))
print('infer:',str(args.infer))
print('gener:',str(args.gener))
...
trainloader = torch.utils.data.DataLoader(train_dataset, batch_size = 1, shuffle = True, drop_last = False, generator = torch.Generator(device='cuda'))

if args.constraint == 'unconstrained':
    paramPath = f'./checkpoints/worm_vae_neuron_held_out_{args.neuron_holdout}_train_worm_{args.train_worm}_{args.model_type}_weight_rand_{str(args.random_init_index)}_infer_{args.infer}_gener_{args.gener}_checkpoint_epoch_299.pt'
    network.load_state_dict(torch.load(paramPath))
...
print(f"用时: {train_end - train_start:.4f} 秒")
