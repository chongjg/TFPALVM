...
class Worm_Inference_Network(nn.Module):
    ...
    def enforce_time_equal(self):
        with torch.no_grad():
            for conv_layer in [self.conv2d1, self.conv2d2, self.conv2d3]:
                W = conv_layer.weight
                W_mean = W.mean(dim=-1, keepdim=True)
                conv_layer.weight[:] = W_mean.repeat(1, 1, W.shape[2], 1)

    def __init__(self, neuron_num, inputs_channels, channels_dict, kernel_size_dict, scale_factor_dict, nonlinearity = F.relu, infer = 0):
        # convolution without padding, expand inputs beforehand to be cropped
        super(Worm_Inference_Network, self).__init__()
        self.infer = infer
        if infer:
            self.conv2d_channels = [4, 8]
            self.conv2d1 = torch.nn.Conv2d(in_channels = 2,                        out_channels = self.conv2d_channels[0], kernel_size = (1, kernel_size_dict['conv1']))
            self.conv2d2 = torch.nn.Conv2d(in_channels = self.conv2d_channels[0],  out_channels = self.conv2d_channels[1], kernel_size = (1, kernel_size_dict['conv2']))
            self.conv2d3 = torch.nn.Conv2d(in_channels = self.conv2d_channels[1],  out_channels = 1,                       kernel_size = (1, kernel_size_dict['conv4']))

            self.conv1d1 = torch.nn.Conv1d(in_channels = neuron_num,                               out_channels = channels_dict['conv3'], kernel_size = kernel_size_dict['conv3'])
            self.conv1d2 = torch.nn.Conv1d(in_channels = (2+self.conv2d_channels[1]) * neuron_num, out_channels = neuron_num,             kernel_size = kernel_size_dict['conv4'])
            self.conv1d3 = torch.nn.Conv1d(in_channels = (2+self.conv2d_channels[1]) * neuron_num, out_channels = neuron_num,             kernel_size = kernel_size_dict['conv5'])
        else:
    ...
    def forward(self, fluorescence_trace_target, missing_target_mask, sensory_input, is_Training = True):
        ...
        # self.enforce_time_equal()

        if self.infer:
            y2d1 = self.nonlinearity(self.conv2d1(inputs.view(1, 2, 300, -1)))
            up2d1 = self.upsample1(y2d1.view(1, self.conv2d_channels[0]*300, -1)).view(1, self.conv2d_channels[0], 300, -1)
            y2d2 = self.nonlinearity(self.conv2d2(up2d1))
            up2d2 = self.upsample2(y2d2.view(1, self.conv2d_channels[1]*300, -1))

            sensory = self.nonlinearity(self.conv1d1(sensory_input))
            merge = torch.cat((up2d2, sensory), 1)

            voltage_latent_mu = self.conv1d2(merge) + self.conv2d3(up2d2.view(1, self.conv2d_channels[1], 300, -1)).view(1, 300, -1)
            voltage_latent_std =  0.1 * F.softplus(self.conv1d3(merge)) + self.eps
        else:
        ...
        if is_Training:
            ...
            voltage_latent_sample = voltage_latent_mu
...
class WormVAE(nn.Module):
    ...
    def __init__(self, connectome, window_size, initialization, upsample_factor, signs_c, encoder, inference_network, device, dt, nonlinearity, model_type, constraint, gener = 0):
        ...
        if gener:
            cnctm_dict = {'sparsity_c': connectome.synapses_dict['chem_adj'],
                          ...
                          'n': connectome.N,
                          'Eleaky': connectome.Eleaky,
                          'Ejinit': connectome.Ejinit,
                          'Eexc': connectome.Eexc,
                          'Einh': connectome.Einh}
        else:
            cnctm_dict = {'sparsity_c': connectome.synapses_dict['chem_adj'],
                          'sparsity_e': connectome.synapses_dict['esym_adj'],
                          'signs_c': signs_c,
        ...
        self.calcium_to_fluorescence_filter = cnctmnn.NeuronWiseLinear(in_features = connectome.N, gener = gener)
        self.loss = ELBO_loss(connectome.N, window_size, self.upsample_factor, gener = gener)
        if gener:
            self.calcium_tau = Parameter(torch.normal(torch.tensor(0.1), 0.01))
        else:
            self.calcium_tau = Parameter(torch.rand(1))
        
    def forward(self, fluorescence_full_target, fluorescence_target, missing_target_mask, odor_input, is_Training = True, OnlyLatent = False):
        if OnlyLatent:
