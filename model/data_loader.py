...
class TimeSeriesDataloader4Test(Dataset):
    def __init__(self, data_param_dict, data, window_size):
        super(TimeSeriesDataloader4Test, self).__init__()
        self.data = data
        self.window_size = window_size
        self.crop_target = int((data_param_dict['k1'] - 1 + (data_param_dict['k2'] - 1)/data_param_dict['up1_factor'] + (data_param_dict['k4'] - 1)/(data_param_dict['up1_factor'] * data_param_dict['up2_factor']))/2)
        self.crop_stim_feature = int((data_param_dict['k3'] - 1   + data_param_dict['k4'] - 1)/2)
        self.max_index0 = (self.data[0].shape[2] - 2 * self.crop_stim_feature) // self.window_size[0]
        self.max_index = self.data[0].shape[0] * self.max_index0
        
    def __getitem__(self, index):
        iworm = index // self.max_index0
        index %= self.max_index0
        return self.data[0][iworm, :, (index) * self.window_size[0]:(index+1) * self.window_size[0] + 2 * self.crop_stim_feature],self.data[1][iworm, :, (index) * self.window_size[1]:(index + 1) * self.window_size[1] + 2 * self.crop_target],self.data[2][iworm, :, (index) * self.window_size[2]:(index+1) * self.window_size[2] + 2 * self.crop_target],self.data[1][iworm, :, (index) * self.window_size[1] + self.crop_target : (index+1) * self.window_size[1] + self.crop_target],self.data[2][iworm, :, (index) * self.window_size[2] + self.crop_target : (index+1) * self.window_size[2] + self.crop_target]
