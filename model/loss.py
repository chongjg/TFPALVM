...
class ELBO_loss(torch.nn.Module):
    ...
    def __init__(self, N, window_size, upsample_factor, gener = 0):
        super(ELBO_loss, self).__init__()
        self.window_size = window_size
        self.upsample_factor = upsample_factor
        self.eps = 1e-8
        self.prior_voltage_mu_init = Parameter((torch.rand(1,N)-2.25) if gener else (2*torch.rand(1,N)-1))
        self.prior_voltage_logvar_init = Parameter(2*torch.rand(1,N)-1)
