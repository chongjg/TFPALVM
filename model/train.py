    ...
    for epoch in range(lr_params['epochs']):
        ...
        for odor_input_window, target_window_padding, missing_window_padding, target_window, missing_window in data_loader:
            ...
            with torch.no_grad():
                ...
                if network.network_dynamics.gener:
                    network.network_dynamics.magnitude_scaling_factor_chem.data.clamp_(min = 1e-2)
                    network.network_dynamics.magnitude_scaling_factor_elec.data.clamp_(min = 1e-2)
                    
                    network.network_dynamics.excRatio[network.network_dynamics.excMask] = 1
                    network.network_dynamics.excRatio[network.network_dynamics.inhMask] = 0
                    network.network_dynamics.excRatio.data.clamp_(min=0.0, max=1.0)
